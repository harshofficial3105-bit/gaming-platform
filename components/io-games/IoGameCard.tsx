...
import { Users, Zap, Flame, Shield, Trophy } from 'lucide-react';
...
export function IoGameCard({ game, playerCount }: IoGameCardProps) {
  ...
  return (
    <div className="group relative flex flex-col justify-between rounded-3xl border border-indigo-200/80 dark:border-indigo-900/60 bg-gradient-to-b from-white via-indigo-50/30 to-white dark:from-[#0E152C] dark:via-[#090D1C] dark:to-[#0A0F22] p-3 sm:p-3.5 shadow-lg shadow-indigo-950/5 dark:shadow-indigo-950/40 hover:shadow-2xl hover:shadow-indigo-500/20 dark:hover:shadow-indigo-500/30 hover:border-indigo-400 dark:hover:border-indigo-500 transition-all duration-300 transform hover:-translate-y-1 overflow-hidden">
      
      {/* Subtle Energy Ambient Glow behind card on hover */}
      <div className="absolute -top-12 -right-12 h-32 w-32 rounded-full bg-indigo-500/10 dark:bg-indigo-500/20 blur-2xl group-hover:bg-cyan-500/20 transition-all pointer-events-none" />
      ...
      <div className="relative w-full aspect-[16/10] rounded-2xl overflow-hidden bg-slate-100 dark:bg-[#050811] border border-indigo-100 dark:border-indigo-950/80">
        ...
        {/* Live Multiplayer Mode Badge (Top Left) */}
        <div className="absolute top-2 left-2 z-10">
          <span className="flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-indigo-950/85 backdrop-blur-md border border-indigo-400/50 text-indigo-200 shadow-md">
            ...
            <span>{multiplayerLabel}</span>
          </span>
        </div>

        {/* Trending Flame (Top Right) */}
        {game.isTrendingIo && (
          <div className="absolute top-2 right-2 z-10">
            <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-mono font-bold bg-rose-950/85 backdrop-blur-md border border-rose-500/50 text-rose-300 shadow-md">
      ...
      <div className="pt-3 pb-1 space-y-2 relative z-10">
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="px-2 py-0.5 rounded-lg text-[10px] font-mono font-bold bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300">
            ★ {game.rating || 4.9}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between font-mono text-[11px] pt-0.5">
          ...
          {game.serverRegions && game.serverRegions[0] && (
            <span className="text-[9px] text-slate-500 dark:text-slate-400">
              {game.serverRegions[0]}
            </span>
          )}
        </div>
